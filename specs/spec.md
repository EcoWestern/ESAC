...
> **Status: normative.** This is the ESAC specification, and it is the only copy that
> governs. Every suite in the program is specified here, and every future amendment is
> appended to this document rather than to a suite repository.
>
> It was written while ESAC-GI was the only suite and lived in that repository until
> 2026-09-22, when it moved to `EcoWestern/ESAC`. Suite repositories cite it as
> `ESAC spec §N` or as `first-amendment §N`; **Part II governs wherever it disagrees with
> Part I**, and the [supersession map](#supersession-map) collects those relationships.
...
rather than merely close. Part II Â§5 replaces the original proposal here, which was one
...
Part II Â§3 replaces the 70/30 framing above with a content-balanced split, and Â§4
...
Part II Â§6 fixes the exact item distribution, and Â§2 replaces the last category.
...
than a separate capability category, described in Part II Â§1.
...
- **Ensemble judging.** Whether rubric items should be scored by one judge model or by
  ...
  categories. *Status: resolved by Part II Â§5, which adopts repeated judging rather than
  voting.*
...
`first-amendment Â§N`, which means the numbered section below.
